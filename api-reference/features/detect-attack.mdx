Glaider helps identify and prevent potential prompt injection attacks that could manipulate AI model behavior. It provides options for asynchronous processing, tagging, session tracking, message saving, strictness levels, and notifications to enhance functionality and integration with your applications.
...
### Detecting Prompt Injection with Strictness Level (Synchronous)
...
def detect_prompt_injection(prompt, strictness_level):
...
strictness_level = 1
result = detect_prompt_injection(prompt, strictness_level)
print("Result:", result)
```

### Detecting Prompt Injection with Asynchronous Processing and Strictness Level
...
You have a chat application and want to analyze user prompts for potential prompt injection attacks with varying levels of strictness based on the user's role or the sensitivity of the conversation. By adjusting the `strictness` parameter, you can control the trade-off between detection accuracy and processing time. For high-risk interactions, you might set `strictness` to `2` to always perform advanced detection, ensuring maximum security.
